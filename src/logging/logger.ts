 ...
 *
 * `resolveRedactOptions()` with no arguments falls through to `resolveConfigRedaction()`,
 * which calls `loadConfig()` directly and may persist `ownerDisplaySecret` as a side
 * effect. This function takes the safer path: it reads logging config via
 * `readLoggingConfig()` (which already honours `shouldSkipMutatingLoggingConfigRead()`),
 * then forwards the result as an *explicit* `RedactOptions` argument so
 * `resolveRedactOptions` never reaches `resolveConfigRedaction()`.
 */
